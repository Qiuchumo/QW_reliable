...
with pm.Model() as partial_model:
    ...
    # switchpoint = pm.DiscreteUniform('switchpoint', lower=0, upper=Num_5)
    # tau1 = pm.Normal('tau1', 0.1, 1, shape=Num_5)
    tau1 = pm.Normal('tau1', 0.1, 1)
...
low1 = low[:k1]
high1 = high[:k1]
aaa = pp_trace['Observed'].mean(axis=0)
...
fig, ax = plt.subplots(figsize=(8, 6))
x_plot1 = np.linspace(1, 7, Num)
ax.fill_between(x_plot, low, high, color=red, alpha=0.5)
ax.plot(x_plot, aaa, c=red, label="Spline estimate")
ax.set_xlim(0, 8)
ax.legend()
plt.show()
