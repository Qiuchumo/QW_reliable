...
error0 = np.max(elec_data.Nums.values[:42]/np.max(elec_data.Nums.values)) # 误差项
error1 = np.max(elec_data.Nums.values[43:91]/np.max(elec_data.Nums.values)) # 误差项
...
with pm.Model() as mulpartial_model:
    ...
    # define likelihood 建立与时间相关的函数
    theta = beta[companyABC] + beta1*elec_year + beta2*elec_tem1 + beta3*elec_hPa1 + beta4*elec_RH1
    Observed = pm.StudentT("Observed", mu=theta, sd=sigma, nu=nu, observed=elec_faults1)  # 观测值
