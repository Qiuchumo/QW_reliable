...
# ======================================================================
dag_data = np.genfromtxt("E:/Code/Bayescode/QW_reliable/Second_model/png/XZmulti_3.csv",
...
with pm.Model() as mulpartial_model:
    ...
    # define likelihood 建立与时间相关的函数
    theta = beta[companyABC] + beta1*elec_year + beta2[companyABC]*elec_tem1 + beta3[companyABC]*elec_hPa1 + beta4*elec_RH1
    Observed = pm.Normal("Observed", mu=theta, sd=sigma, observed=elec_faults1)  # 观测值

    start = pm.find_MAP()
    # step = pm.Metropolis()
    trace3 = pm.sample(6000, start=start, tune=2000)
chain3 = trace3
varnames1 = ['beta', 'beta1', 'beta2', 'beta3', 'beta4']
