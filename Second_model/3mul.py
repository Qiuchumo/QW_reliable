...
with pm.Model() as mulpartial_model:
    ...
    # define likelihood 建立与时间相关的函数
    # theta = pm.math.logit(beta[companyABC] + beta1*elec_year + beta2*elec_tem1)
    theta = beta[companyABC] + beta1*elec_year1*elec_year1 + beta2*elec_tem1 + beta3*elec_RH1
    # theta1 = pm.Deterministic('theta1', theta)
    Observed = pm.Normal("Observed", mu=theta, sd=sigma, observed=elec_faults1)  # 观测值
    # Observed = pm.Lognormal("Observed", mu=theta, sd=sigma, observed=elec_faults1)  # 观测值

    start = pm.find_MAP()
    # step = pm.Metropolis()
    trace3 = pm.sample(3000, start=start, tune=1000)
chain3 = trace3
# varnames1 = ['beta', 'beta1']
# pm.traceplot(chain3, varnames1)
# plt.show()
varnames1 = ['sigma', 'beta', 'beta1', 'beta2', 'beta3']
pm.traceplot(chain3)
plt.show()
# 画出自相关曲线
