...
with pm.Model() as unpooled_model:
    ...
    beta = pm.Normal('beta', 0, 10000)
    beta1 = pm.Normal('beta1', 0, 10000)
    beta2 = pm.Normal('beta2', 0, 1000)

    # define likelihood 建立与时间相关的函数
    mu = pm.Deterministic('mu', tt.exp(beta + beta1 * elec_year + beta2*elec_tem))
    Observed_pred = pm.Poisson("Observed_pred", mu=mu, shape=elec_faults.shape)  # 观测值
    Observed = pm.Poisson("Observed", mu=mu,  observed=elec_faults)  # 观测值
    # Observed_pred = pm.Normal("Observed_pred", theta, sigma, shape=elec_faults.shape)  # 观测值
    # Observed_pred = pm.Bound(pm.Normal, lower=0.0)('Observed_pred', mu=theta, sd=sigma, shape=elec_faults1.shape)  # 观测值
    start = pm.find_MAP()
    # step1 = pm.Slice([beta, beta1])
    # step1 = pm.Metropolis()
    # trace2 = pm.sample(2000, step=step1, start=start)
...
# plt.hist(com_pred,  range=[-2, 10], bins=90, histtype='stepfilled', color='#6CA6CD')
# plt.xlabel(u"时间t/年", fontsize=14, fontproperties=font)
...
#     j = j + k1
# ax.plot(elec_year[0:6], tt.exp(org_beta + org_beta1 * elec_year[0:6] + org_beta2 * elec_tem[0:6]), 'b-', linewidth=4)
...
# #         plt.plot(elec_year[j:(j + k1)], elec_faults[j:(j + k1)], 'k--', linewidth=0.9)
# #     j = j + k1
# ax.plot(elec_year[42:49], tt.exp(org_beta0 + org_beta11 * elec_year[42:49] + org_beta2 * elec_tem[42:49]), 'b-', linewidth=4)
...
# #         plt.plot(elec_year[j:(j + k1)], elec_faults[j:(j + k1)], 'k--', linewidth=0.9)
# #     j = j + k1
# ax.plot(elec_year[91:97], tt.exp(org_beta00 + org_beta111 * elec_year[91:97] + org_beta2 * elec_tem[91:97]), 'b-', linewidth=4)
