...
sns.kdeplot(elec_faults1)
plt.hist(elec_faults1, range=[-1, 5], bins=130, histtype='stepfilled', color='#6495ED')
plt.axvline(elec_faults1.mean(), color='r', ls='--', label='True mean')
plt.show()
# # 画出原始图
...
with pm.Model() as unpooled_model:
    ...
    mu = pm.Deterministic('mu', tt.exp(beta[companyABC] + beta1[companyABC] * elec_year1 + beta2 * elec_tem1))
    ...
    Observed = pm.NegativeBinomial("Observed", mu=mu, alpha=sigma, observed=elec_faults1)  # 观测值
