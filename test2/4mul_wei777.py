...
with pm.Model() as unpooled_model:
    ...
    beta1 = pm.Normal('beta1', 0, 100000)
...
plt.show()
''''''
# 删掉第七组数据的情况，数据完全不集中模型
with pm.Model() as unpooled_model:
    sigma = pm.HalfCauchy('sigma', 10, testval=1.)

    beta = pm.Normal('beta', 0, 100000, shape=companiesABC)
    beta1 = pm.Normal('beta1', 0, 100000, shape=companiesABC)
    beta2 = pm.Normal('beta2', 0, 100000, shape=companiesABC)
    # mu = tt.exp(beta[companyABC] + beta1[companyABC]*elec_year + beta2*elec_tem)
    mu = pm.Deterministic('mu', tt.exp(beta[companyABC] + beta1[companyABC]*elec_year + beta2[companyABC]*elec_tem))
    # Observed_pred = pm.Weibull("Observed_pred",  alpha=mu, beta=sigma, shape=elec_faults.shape)  # 观测值
    Observed = pm.Weibull("Observed", alpha=mu, beta=sigma, observed=elec_faults)  # 观测值
...
with pm.Model() as unpooled_model:
    ...
    beta_deta01 = pm.Deterministic('beta_deta01', beta1[1] - beta1[0])
    beta_deta02 = pm.Deterministic('beta_deta02', beta1[2] - beta1[0])
    beta_deta12 = pm.Deterministic('beta_deta12', beta1[2] - beta1[1])
...
chain2 = trace2[1000:]
varnames1 = ['beta_deta01', 'beta_deta02', 'beta_deta12']
...
bbb1 = pd.DataFrame(aaa)

hpd2_5 = bbb1['hpd_2.5']
hpd97_5 = bbb1['hpd_97.5']
