...
with pm.Model() as unpooled_model:
    ...
    beta = pm.Normal('beta', 0, 10, testval=-3.)
    beta1 = pm.Normal('beta1', 0, 10, shape=companiesABC, testval=.3)
    beta2 = pm.Normal('beta2', 0, 10, testval=0.01)
    beta3 = pm.Normal('beta3', 0, 10, shape=companiesABC)
    θ = pm.Gamma('θ', 1, 1000)
    u = pm.Normal('u', 0, θ*θ, shape=companiesABC)
    # mu = tt.exp(beta[companyABC] + beta1[companyABC]*elec_year + beta2*elec_tem)
    beta_mu = pm.Deterministic('beta_mu', tt.exp(beta + beta1[companyABC] * x_shared + beta2 * x_shared1 \
                                                 + beta3[companyABC] * x_shared * x_shared + u[companyABC]))
...
varnames2 = ['beta', 'beta1', 'beta2', 'u', 'alpha', 'beta3']
...
hpd25_alpha = hpd2_5[8:9].mean()
hpd975_alpha = hpd97_5[8:9].mean()

hpd25_beta3_0 = hpd2_5[9:10].mean()
hpd975_beta3_0 = hpd97_5[9:10].mean()
...
post_u1 = hpdmean[6]
post_u2 = hpdmean[7]
post_alpha = hpdmean[8]
post_beta30 = hpdmean[9]
post_beta31 = hpdmean[10]
post_beta32 = hpdmean[11]

# 计算平均故障率，即在时间与温度双重作用下的频率故障率变化
betaa = np.exp(post_beta + post_beta1 * elec_year[0:6] + post_beta2 * elec_tem[0:6] + post_beta30*elec_year[0:6]*elec_year[0:6] + post_u0)
betaa1 = np.exp(post_beta + post_beta11 * elec_year[42:48] + post_beta2 * elec_tem[42:48] + post_beta31*elec_year[0:6]*elec_year[0:6] + post_u1)
betaa2 = np.exp(post_beta + post_beta111 * elec_year[84:90] + post_beta2 * elec_tem[84:90] + post_beta32*elec_year[0:6]*elec_year[0:6] + post_u2)
Mean_gamma = betaa * gamma((1 + 1 / post_alpha))
Mean_gamma1 = betaa1 * gamma((1 + 1 / post_alpha))
Mean_gamma2 = betaa2 * gamma((1 + 1 / post_alpha))

# 计算后验均值区 间97.5
betaa975  = np.exp(hpd975_beta + hpd975_beta1_0 * elec_year[0:6] + hpd975_beta2 * elec_tem[0:6] + hpd975_beta3_0*elec_year[0:6]*elec_year[0:6] + hpd975_u_0)
betaa9751 = np.exp(hpd975_beta + hpd975_beta1_1 * elec_year[42:48] + hpd975_beta2 * elec_tem[42:48] + hpd975_beta3_1*elec_year[0:6]*elec_year[0:6] + hpd975_u_1)
betaa9752 = np.exp(hpd975_beta + hpd975_beta1_2 * elec_year[84:90] + hpd975_beta2 * elec_tem[84:90] + hpd975_beta3_2*elec_year[0:6]*elec_year[0:6] + hpd975_u_2)
...
# 计算后验均值区间2.5
betaa25  = np.exp(hpd25_beta + hpd25_beta1_0 * elec_year[0:6] + hpd25_beta2 * elec_tem[0:6] + hpd25_beta3_0*elec_year[0:6]*elec_year[0:6] + hpd25_u_0)
betaa251 = np.exp(hpd25_beta + hpd25_beta1_1 * elec_year[42:48] + hpd25_beta2 * elec_tem[42:48] + hpd25_beta3_1*elec_year[0:6]*elec_year[0:6] +hpd25_u_1)
betaa252 = np.exp(hpd25_beta + hpd25_beta1_2 * elec_year[84:90] + hpd25_beta2 * elec_tem[84:90] + hpd25_beta3_2*elec_year[0:6]*elec_year[0:6] +hpd25_u_2)
